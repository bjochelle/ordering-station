<template>
    <div class="card overflow-hidden">
            ...
            <div class="row gx-0 align-items-center">
                <div class="col-auto d-flex justify-content-end order-md-1">
                    <button class="btn btn-primary btn-sm"
                            type="button" @click="showCreateModal">New Role
                    </button>
                </div>
        ...
        <div class="card-body bg-light px-1 py-0 ">
            <PulseLoading v-if="isLoading.roles"></PulseLoading>
            <div v-else class="table-responsive table-responsive-sm scrollbar">
                <table class="table overflow-hidden fs--1">
                    ...
                    <tr>
                        ...
                        <th>Permissions</th>
                        <th class="text-nowrap" @click="sort('guard_name')">
                            <div class="inline-block">
                                <div :class="{'active-filter-asc':table.sortClassActive.activeAsc}"
                                     class="filter-asc"></div>
                                <div :class="{'active-filter-desc':table.sortClassActive.activeDesc}"
                        ...
                        <th class="text-nowrap" @click="sort('created_at')">
                            <div class="inline-block">
                                <div :class="{'active-filter-asc':table.sortClassActive.activeAsc}"
                                     class="filter-asc"></div>
                                <div :class="{'active-filter-desc':table.sortClassActive.activeDesc}"
                    ...
                    <tbody>
                    <tr v-for="(role) in roles.data" :key="role.id" class="align-middle">
                        <td class="text-nowrap">{{ role.name }}</td>
                        <td>
                            <span v-for="(permission) in role.permissions"
                                  class="badge rounded-pill fs--2 bg-200 text-primary">
                                {{ permission.name }}
                            </span>
                        </td>
                        ...
                        <td class="text-nowrap">
                            <template v-if="role.name  !== 'Super Administrator'">
                                ...
                                <button aria-label="Delete" class="btn p-0 shadow-none" data-bs-original-title="Delete"
                                        data-bs-placement="top" data-bs-toggle="tooltip" type="button"
        ...
        <div class="card-footer">
            <Bootstrap5Pagination :data="roles" :limit="20" :show-disabled="true" align="right" size="small"
    ...
    <ModalComponent ref="thisModal" :backdrop="modal_properties.backdrop" :keyboard="modal_properties.keyboard"
        ...
        <template #body>
            <CreateRole v-if="modal_properties.type==='create'"></CreateRole>
            <EditRole v-if="modal_properties.type==='edit'"></EditRole>
        </template>
        <template #footer>
            <button v-if="modal_properties.type==='create'" class="btn btn-sm btn-primary me-1 mb-1"
                    @click="submitData">Submit
            </button>
            <button v-if="modal_properties.type==='edit'" class="btn btn-sm btn-info me-1 mb-1"
...
import CreateRole from "@/layout/views/users_management/roles/CreateRole.vue";
import EditRole from "@/layout/views/users_management/roles/EditRole.vue";
...
const {roles, isLoading, role} = storeToRefs(roleStore)//user variable from store
const {fetchRolesForDatatable, createRole, fetchRole, updateRole, removeRole} = roleStore; //user methods from store
...
const showDeleteModal = (id) => {
    swal({
        ...
        if (result.isConfirmed) {
            ...
            // let data_index = roles.value.data.findIndex(x => x.id === id)
            // roles.value.data.splice(data_index, 1);
            await removeRole(id)
            swal({
                icon: 'success',
                title: 'Deleted',
...
const showCreateModal = async () => {
    ...
    modal_properties.value.title = 'Create Role'
...
const showEditModal = async (id) => {
    ...
    modal_properties.value.title = 'Edit Role Details'
    modal_properties.value.backdrop = 'static'
    modal_properties.value.keyboard = true
...
const dataTable = _.debounce(function (page) {
    ...
    fetchRolesForDatatable(table, pages);
}, 800);
